...
public class DeriveRequestService {
	...
	public GenFile getDerivedGenFileByWidth(DeriveRequest deriveRequest, int width) {
		...
		return genFileService.getGenFileByRelTypeCodeAndRelIdAndFileExtTypeCodeAndWidthAndHeight("deriveRequest", deriveRequest.getId(), "img", width, height);
	}
	...
	public GenFile getDerivedGenFileByMaxWidth(DeriveRequest deriveRequest, int maxWidth) {
		...
		return genFileService.getGenFileByRelTypeCodeAndRelIdAndFileExtTypeCodeAndWidthAndHeight("deriveRequest", deriveRequest.getId(), "img", maxWidth, height);
	}
	...
	private GenFile makeDerivedGenFileByWidthAndHeight(DeriveRequest deriveRequest, int width, int height) {
		...
		ResultData saveRd = genFileService.save("deriveRequest", deriveRequest.getId(), "common", "derived", 0, originGenFile.getOriginFileName(), destFilePath);
